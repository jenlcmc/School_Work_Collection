...
double distanceTraveled( vector<AISType> & dataBase, int first, int last ){
    ...
    double First_Lat_Radian = (dataBase[first].lattitude * PI) / 180;
    double Second_Lat_Radian = (dataBase[last].lattitude * PI) / 180;
    double LattitudeChange = Second_Lat_Radian - First_Lat_Radian;
    double LongitudeChange = ((dataBase[last].longitude - dataBase[first].longitude) * PI) /180;

    double a = pow(sin(LattitudeChange / 2), 2) + cos(First_Lat_Radian * 1) *
               cos(Second_Lat_Radian * 2) * pow(sin(LongitudeChange / 2), 2);
